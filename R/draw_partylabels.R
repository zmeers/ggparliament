...
#' @param party_colors A column containing party colors.
...
draw_partylabels <- function(type = c("semicircle", "horseshoe"),
  ...
  structure(
    list(
      ...
      party_colors = rlang::enquo(party_colors)
    ),
    class = "partyLabels"
  )
}
...
ggplot_add.partyLabels <- function(object, plot, object_name) {
  ...
  group_data <- dplyr::group_by(filt_data, pn = !!object$party_names, ps = !!object$party_seats, pc = !!object$party_colors)
