...
#' @param linecolor The color of the majority line. Defaults to gray.
...
ggplot_add.majorityLine <- function(object, plot, object_name) {
  ...
  if (object$type == "semicircle") {
    if (!object$label) {
      plot +
        ggplot2::geom_segment(ggplot2::aes(y = 0.8, yend = max(plot$data$y) + 0.1, x = 0, xend = 0), color = object$linecolor, size = object$linesize, linetype = object$linetype, alpha = object$linealpha)
    } else {
      plot +
        ggplot2::geom_segment(ggplot2::aes(y = 0.8, yend = max(plot$data$y) + 0.1, x = 0, xend = 0), color = object$linecolor, size = object$linesize, linetype = object$linetype, alpha = object$linealpha) +
    ...
    if (object$type == "horseshoe") {
      if (!object$label) {
        plot +
          ggplot2::geom_segment(ggplot2::aes(y = 7.5, yend = 10.5, x = 0, xend = 0), color = object$linecolor, size = object$linesize, linetype = object$linetype, alpha = object$linealpha)
      } else {
        plot +
          ggplot2::geom_segment(ggplot2::aes(y = 7.5, yend = 10.5, x = 0, xend = 0), color = object$linecolor, size = object$linesize, linetype = object$linetype, alpha = object$linealpha) +
        ...
        if (!object$label) {
          plot +
            ggplot2::geom_segment(aes(y = y_pos_oppbenches + 0.5, yend = y_pos_oppbenches + 0.5, x = min(plot$data$x), xend = max(plot$data$x) / 2),
              color = object$linecolor, size = object$linesize, linetype = object$linetype, alpha = object$linealpha
            )
        } else {
          plot +
            ggplot2::geom_segment(aes(y = y_pos_oppbenches + 0.5, yend = y_pos_oppbenches + 0.5, x = min(plot$data$x), xend = max(plot$data$x) / 2),
