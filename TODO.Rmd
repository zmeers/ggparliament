...
# TODO
## Website
Should we create a pkgdown website?

## Splitting data frame by multiple columns and then plotting different plots
i.e. split by country AND year, then facet_wrap by country and year. Right now you can only plot one choice -- the row number or parliament style is set. 
...
* this kind of comes under the women_in_parliament stuff- I wonder if it's best to do this as an alpha scale which could also be used for stuff like votes. Wonder if it would be possible to infer 'Brexity-ness' from https://cran.r-project.org/web/packages/hansard/index.html (/probably simpler just to take it from theyworkforyou but not as good for a Vignette) and then alpha the parties to show who would vote for e.g. the big brexit vote coming up
  - I like this idea! It'll be a good way to drum up more publicity for the package as well. If we can time CRAN release so that it fits in with the Brexit vote, that'd be pretty cool.
