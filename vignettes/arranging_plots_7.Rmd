...
## Improving Information Converyance via Plot Order
...
german_parliament <- ggplot(formatted_parl_data, aes(x, y, colour = party_short)) +
  ...
  theme_ggparliament() +
  scale_colour_manual(
    values = formatted_parl_data$colour,
...
german_parliament <- unformatted_data %>%
  ...
  theme_ggparliament() +
  scale_colour_manual(
    values = unformatted_data$colour,
    limits = unformatted_data$party_short
  )
german_parliament
```

Given that government is a binary variable, the simplest way to order as in the first plot is to multiply this by the number of seats, i.e.:
